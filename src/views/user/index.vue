<template>
  <div class="px-[16px] pb-[16px]">
    ...
    <VanList
        ...
        @load="onLoad"
    >
      <Item v-for="item in state.list" :key="item" :item="item"></Item>
    </VanList>
  </div>
</template>
...
import {subordinateList} from "@/services/share";
...
const getList = async () => {
  const resp: any = await subordinateList({
    page: state.page,
    size: state.size,
  })
  if (!resp.success) {
